var theGame = function(game) {
    ...
    trumpRemarks = ['trumpRemark1'];
    hillaryRemarks = ['hillaryRemark1'];
    obamaRemarks = ['obamaRemark1'];
    bernieRemarks = ['bernieRemark1'];
    jebRemarks = ['jebRemark1'];
    bushRemarks = ['bushRemark1'];
...
theGame.prototype = {
    create: function() {
        ...
        trackLives = this.game.add.sprite(this.game.world.width - 90, 60, 'life');
    ...
    update: function() {
        ...
        this.game.physics.arcade.overlap(bullets, inventoryItemsGroup, this.collectSingleInventoryItem, null, this);
    ...
    createStars: function() {
        ...
        for (var i = 0; i < levelData.levelsData.levels[indexedLevel].starNumbers; i++) {
            ...
            var star = stars.create(i * 70, 0, 'orb');
    ...
    collectSingleInventoryItem: function(player, item) {
        ...
        healthpacksound.volume = 1;
        healthpacksound.play();

        // REMARK SOUND
        var remarkNumber = this.game.rnd.integerInRange(0, 0) + 1;
        var remarkSound = this.game.add.audio('hillaryRemark' + remarkNumber);
        //console.log('hillaryRemark' + remarkNumber);
        console.log("12 hit");
        remarkSound.volume = sfxVolume;
        remarkSound.play();
    ...
    gameEnd: function() {
        ...
        if (remainingLives != 0 && rocksYouCollected >= levelData.levelsData.levels[indexedLevel].rocksNeeded) {
            ...
            if (indexedLevel == levelData.levelsData.levels.length) {
                ...
                if (remainingLives >= 1 && indexedLevel == 9 && bonusLevel != bonusLevelData.bonuslevelsData.levels.length) {
    ...
    onGameWin: function() {
        ...
        highScore.events.onInputUp.add(this.quitGame, this);
        pause_label.events.onInputUp.removeAll();
        stars.removeAll();
        rocks.removeAll();
        //endScoreText.anchor.setTo(0.5, 0.5);
    ...
    saveScores: function() {
        // save score
        var playerScores = [];
        playerScores = JSON.parse(localStorage.getItem('presidency_highscores'));
        if (localStorage.getItem('presidency_highscores') !== null && score > 0) {
            ...
            if (score > parseInt(localStorage.getItem('presidency_bestScore')) || localStorage.getItem('presidency_bestScore') == "") {
                localStorage.setItem('presidency_bestScore', score);
            }

            playerScores.sort(function(a, b){return a-b}).reverse();
            console.log("scores", playerScores);

            localStorage.setItem('presidency_highscores', JSON.stringify(playerScores));

        }

        // set inventory items
        if (localStorage.getItem('presidency_inventory') !== null) {
            var itemsStored = JSON.parse(localStorage.getItem('presidency_inventory'));


            itemsStored.data.bernie += bernieCollected;
            itemsStored.data.bill += billCollected;
            itemsStored.data.bush += bushCollected;
            itemsStored.data.jeb += jebCollected;
            itemsStored.data.obama += obamaCollected;
            itemsStored.data.orbs += orbsCollected;


            var saveJSON = '{"data":{"bernie":' + itemsStored.data.bernie + ',"bill":' + itemsStored.data.bill + ',"bush":' + itemsStored.data.bush + ',"jeb":' + itemsStored.data.jeb + ',"obama":' + itemsStored.data.obama + ',"orbs":' + itemsStored.data.orbs + '}}';
        ...
        scoreBoard = this.game.add.text(
            w / 2, 100,
            'Your Scores: \n' + this.displayScoreLayout(playerScores, localStorage.getItem('presidency_bestScore')), {
                font: '30px Helvetica',
                fill: '#fff'
            }
        );
    ...
    displayScoreLayout: function(playerScores, playerBest) {
        ...
        // collection items
        var bernieCol = this.game.add.sprite(w/2 - 280, 460, 'bernie');
        collectedItemsData.add(bernieCol);
        var obamaCol = this.game.add.sprite(w/2 - 170, 460, 'obama');
        collectedItemsData.add(obamaCol);
        var billCol = this.game.add.sprite(w/2 - 60, 460, 'bill');
        collectedItemsData.add(billCol);
        var bushCol = this.game.add.sprite(w/2 + 60, 460, 'bush');
        collectedItemsData.add(bushCol);
        var jebCol = this.game.add.sprite(w/2 + 170, 460, 'jeb');
        collectedItemsData.add(jebCol);
        var orbCol = this.game.add.sprite(w/2 + 280, 460, 'orb');
        collectedItemsData.add(orbCol);

        // text displayed underneath of gains
        var bernieText = this.game.add.text(w/2 - 280 - 2, 500,
            "+ " + bernieCollected, {
                font: '18px Helvetica',
                fill: '#fff'
            },
        ...
        var obamaText = this.game.add.text(w/2 - 170 - 2, 500,
            "+ " + obamaCollected, {
                font: '18px Helvetica',
                fill: '#fff'
            },
        ...
        var billText = this.game.add.text(w/2 - 60 - 2, 500,
            "+ " + billCollected, {
                font: '18px Helvetica',
                fill: '#fff'
            },
        ...
        var bushText = this.game.add.text(w/2 + 60 - 2, 500,
            "+ " + bushCollected, {
                font: '18px Helvetica',
                fill: '#fff'
            },
        ...
        var jebText = this.game.add.text(w/2 + 170 - 2, 500,
            "+ " + jebCollected, {
                font: '18px Helvetica',
                fill: '#fff'
            },
